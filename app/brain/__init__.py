...
def change_password():
    ...
    p = subprocess.call('bash {} "{}"'.format(str(os.environ.get('VIRTUALSKIN_BRAIN_ADMIN')) + '/change_password.sh', password + "\\n" + password), shell=True)
    result = {"status": p}
    # result = {"status": 0}
    return app.response_class(response=json.dumps(result), status=c.OK, mimetype=c.JSON)
...
def subscribe():
    ...
    p = subprocess.call('bash {} {}'.format(str(os.environ.get('VIRTUALSKIN_BRAIN_ADMIN')) + '/ssh-copy.sh', ip), shell=True)
    if not p:
        p = subprocess.call('bash {} {}'.format(str(os.environ.get('VIRTUALSKIN_BRAIN_ADMIN')) + '/ssh-config.sh', ip), shell=True)
    result = {"status": p}
    return app.response_class(response=json.dumps(result), status=c.OK, mimetype=c.JSON)
